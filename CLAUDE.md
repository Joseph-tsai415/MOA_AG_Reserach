...
> - **Each step must be delegated to a separate sub-agent** using the Agent tool. This preserves the main context window — the orchestrator only passes inputs and collects outputs, never processes raw API data itself.
